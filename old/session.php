...
class Auth {
  ...
  public function TIn($session)
  {
    ...
    throw new SoapFault('client', 'Error on function');
  }
  ...
  return new SoapParam($response, 'Timestamp');
  }
  ...
  public function Su($session, $level)
  {
  $uid = Auth::GetUID($session);
  if (!$uid) throw new SoapFault('client', 'No session!');

  $q = ("SELECT su FROM su WHERE uid='$uid' AND level='$level'");
  $r = Misc::db_query($q);
  $s = Misc::db_selt($r);
